...
import com.careLink.auth.result.JoinResult;
...
public class AuthController {
    ...
    public JoinResult signup(@RequestBody MemberEntity memberEntity) {
        log.info("일반회원가입 시작");
        memberEntity.join("ROLE_USER",1); //회원권한, 회원탈퇴여부 부여

        int memberId = authService.signUp(memberEntity); //회원가입 성공 후 고유 번호 리턴
    ...
    public LoginResult login(@RequestBody SignInDto signInDto) {
        ...
        log.info("4");
        log.info("로그인 아이디 값 : " + signInDto.getMemberId());
        //AccessToken 생성 ---------------------------------------
